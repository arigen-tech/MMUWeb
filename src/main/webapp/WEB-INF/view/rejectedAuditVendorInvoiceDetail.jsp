...
$(document).ready(function(){
   ...
   /*  $('#back').on('click', function(){
        window.location = 'pendingAuditorVendorInvoice';
    }); */
    ...
    jQuery.ajax({
        ...
        success: function(result){
            if(result && result.vendorInvoiceDetailData && result.vendorInvoiceDetailData.data && result.vendorInvoiceDetailData.data.length > 0){
				...
				if(lastApprovalMsg.startsWith('Back to TPA')){
					 $('#caseSheet1').show()
					  $('#caseSheet2').show()
...
function getAuthorityList(){
	jQuery.ajax({
	    ...
	    success: function(result){
	    	var combo = "" ;
	    	var authId=$('#auditorAuthorityOrderNo').val();
	    	for(var i=0;i<result.masAuthorityData.length;i++){
	    		var checkAuthId=result.masAuthorityData[i].orderNo;
	    		if(authId>checkAuthId && result.masAuthorityData[i].authorityName.startsWith("Nodal"))
	    		{	
			...
			.ajax({
				...
				success : function(response) {
					...
					$.each(datas, function(i, item) {
						...
						trHTML +='<td><div><textarea class="form-control" type="text" maxlength="1000" id="authorityRemarks" readonly>'+authorityRemarks+'</textarea></div></td>'; 
...
function getAuditorDownloadData(button){
    ...
    window.open("${pageContext.servletContext.contextPath}/audit/download?name="+namVal+"&type=audit_report&keys="+$('#invoiceNo').val(), '_blank').focus();
}
...
function submitAuditorForm(val){
	
	  var penaltyMMUId=[];
	   var penaltyData='';
	  /*  var penaltyMMURemarks=[];
	   var penaltyMMUAmount=[]; */
	   $('#mmuPenaltyList tr').each(function(i, el) {
			var $tds = $(this).find('td');
			var repeatId = $tds.eq(0).find(":text").attr("id");
		   
				var itemId=$($tds).closest('tr').find("td:eq(4)").find("input:eq(0)").val();
				 var remarksValue=$($tds).closest('tr').find("td:eq(2)").find("textarea:eq(0)").val();
				 var amount=$($tds).closest('tr').find("td:eq(5)").find("input:eq(0)").val();
				 ...
				 penaltyData={
				      ...
				      penaltyMMUId.push(penaltyData);
				
			});	
	  $('#mmuPenaltyListData').val(penaltyMMUId);
   ...
   if(document.getElementById("actionId").checked == false){
	      alert('Please check I acknowledge that I have read!');
	      return false;
	  }
   if($('#actionId').val()!="R")
   {	   
	  if(!$('#forwordTo').val()){
	      alert('Please Select action checkbox!');
	      return false;
	  }
   }
   var lastApprovalMsg='';
   ...
   if($('#actionId').val()=="A")
   {
   	lastApprovalMsg='Acknowledged and read by Auditor on ('+currentdate+')';
   	
   }
   else
   {
   	lastApprovalMsg='Rejected by Auditor on ('+currentdate+')';
   }	
   $('#lastApprovalMsg').val(lastApprovalMsg);
 
 
  if($('#penaltyAmountDisplay').is(':visible')) {
	  if($('#penaltyAmountImposed').val()==""){
	      alert('Please enter Penalty Amount Imposed!');
	      return false;
	  }
  }
   if($('#fileNameAuditor').val()
       && $('#fileNameAuditor').val().indexOf('.pdf') == -1
       && $('#fileNameAuditor').val().indexOf('.doc') == -1
       && $('#fileNameAuditor').val().indexOf('.docx') == -1
       && $('#fileNameAuditor').val().indexOf('.xls') == -1
       && $('#fileNameAuditor').val().indexOf('.xlsx') == -1){
   ...
   var formData = new FormData(document.getElementById("captureVendorBillAuditorForm"));
   //SendMultipartData('captureVendorBillAuditorDetail',formData);
   //$('#captureVendorBillForm')[0].reset();
  // $('#mmuIds').empty();
   //setTimeout( function() { window.location.href ="vendorInvoiceList"; }, 3000);
   var pathname = window.location.pathname;
   var accessGroup = "MMUWeb";

   var url = window.location.protocol + "//" + window.location.host + "/" + accessGroup + "/audit/captureVendorBillAuditorDetail";
	...
	$.ajax({
       ...
       success: function(msg) {
           ...
           if (msg.status == 1)
           {
              ...
              localStorage.typeOfVal='auditor';
	...
	<div id="wrapper">
		<div class="content-page">
			...
			<div class="container-fluid">
				...
				<div class="internal_Htext">Back to Auditor By Nodal Officer</div>
                <form id="captureVendorBillAuditorForm">
				<div class="row">
                ...
                <input  name="auditorName" id="auditorName" type="hidden" value=""/>
                <input  name="userDesignation" id="userDesignation" type="hidden" value=""/>
                <input  name="auditorAuthorityId" id="auditorAuthorityId" type="hidden" value="<%=session.getAttribute("authorityId")%>"/>
                <input  name="auditorAuthorityName" id="auditorAuthorityName" type="hidden" value="<%=session.getAttribute("authorityName")%>"/>
                <input  name="auditorAuthorityOrderNo" id="auditorAuthorityOrderNo" type="hidden" value="<%=session.getAttribute("approvalOrderNo")%>"/>
                <input  name="billMonth" id="billMonth" type="hidden" value=""/>
                <input  name="lastApprovalMsg" id="lastApprovalMsg" type="hidden" value=""/>
                <input type="hidden" name="mmuPenaltyListData" id="mmuPenaltyListData" value=""/>
                                 ...
                                 <div class="row m-t-10">
									   <div class="col-lg-4 col-sm-6">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<button class="btn btn-primary" id="downloadAuditorReport" onclick="getAuditorDownloadData(this)">View/Download</button>
											</div>
										</div>
									</div>
									<!-- <div class="col-lg-4 col-sm-6">
										<div class="form-group row">
											<div class="col-md-5">
												<label class="col-form-label">Auditor Final Remarks</label>
											</div>
											<div class="col-md-7">
												<div class="dateHolder">
													<textarea name="auditorRemarks"   readonly style="width:600px; height:100px;" maxlength="1000" class="form-control border-input
									...
									<div class="row">
										...
										<div class="col-lg-4 col-sm-6">
											<div class="form-group row">
												<label class="col-md-5 col-form-label">Imposed
									...
									<div class="row">
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-7">
                                                    <!-- <select class="form-control" name="actionId" onchange="disbaledAuditorForwardTo()" id="actionId">
                                                        <option value="">--Select--</option>
                                                        <option value="A">I acknowledge that I have read.</option>
                                                        <option value="R">Reject</option>
                                                    </select> -->
                                                    <label for="acknowledge">
											         <input class="form-check-input" type="checkbox" id="actionId" value="A" onchange="disbaledAuditorForwardTo()" name="actionId" required>
 									 ...
 									 <div class="col-md-4" style="display:none">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<select class="form-control" name="forwordTo" id="forwordTo" readonly>
												<option value="">Select</option>
												</select>
											</div>
										</div>
                                	...
                                	<button type="button" id="captureAuditDataSubmit" value="submit" onclick="submitAuditorForm(this.value)" class="btn btn-primary">Submit</button>
                               		<button name="data" class="btn btn-primary" type="button" id="back" onclick="backToList()">Back</button>
                               		</div>
                          		</div>				
