...
function getCommandList(MODE)
{
	...
	var searchService = $j('#serviceNo').val();
	 ...
	 if(MODE == 'ALL'){
			var data = {"pageNo":nPageNo,"serviceNo":"","userId" : <%=userId%>, 'hospitalId':""+<%=hospitalId%>,'flagForList':'e'};			
		}
	else
		{
			var data =  {"pageNo":nPageNo, "serviceNo":searchService,"userId":<%=userId%>, "hospitalId":""+<%=hospitalId%>,'flagForList':'e'}; 
		} 
...
function executeClickEvent(visitId,data)
{
	...
	rowClick(visitId,patientId,patinetname,rankName,age,gender,mediceExamDate,medicalCategory,meTypeName,approvedBy, status,departmentId,meTypeCode);
}



function rowClick(visitId,patientId,patinetname,rankName,age,gender,mediceExamDate,medicalCategory,meTypeName,approvedBy, status,departmentId,meTypeCode){
		$("#visitId").val(visitId);
		...
		if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() === resourceJSON.meForm18){
			window.location.href = "meApprovalWaitingPerusingDetailF18?visitId="+visitId+"&age="+age+"&approvalFlag=n"+"&userId="+<%=userId%>;
		}
		else if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() == resourceJSON.meForm3A){
			window.location.href = "meApprovalWaitingPerusingDetail3A?visitId="+visitId+"&age="+age+"&approvalFlag=n"+"&userId="+<%=userId%>;	
		}
		else{
			window.location.href = "meApprovalWaitingPerusingDetail?visitId="+visitId+"&age="+age+"&approvalFlag=n"+"&userId="+<%=userId%>;	
		}	
		...
		<div class="content-page">
			...
			<div class="">
				<div class="container-fluid">
					<div class="internal_Htext">ME Waiting List (Perusing Authority)</div>
