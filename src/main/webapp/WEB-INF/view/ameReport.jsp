...
<div id="wrapper">
	<div class="content-page">
		...
		<div class="container-fluid">
			<div class="internal_Htext">AME Report For Officers/Sailors</div>
					...
					<div class="card">
						<div class="card-body">	
								...
								<div class="col-md-4">
									<div class="form-group row">
										<div class="col-md-5">
											<label class="col-form-label">Emp Category</label>
										</div>
										<div class="col-md-7">
 ...
 $j(document).ready(function()
	{
     ...
     for (category in empCategoryData) {
    	 ...
    	 empCategoryValues += '<option  value=' + category + '>' + empCategoryData[category] + '</option>';
     }
     ...
     $j('#medcategory').append(medCategoryValues);
...
function getAmeReportList(MODE)
{
	var fromDate = $j('#fromDate').val();
	var toDate = $j('#toDate').val();
	var category = $j('#category').val();
	var medcategory = $j('#medcategory').val();
	
	var unit = $j('#unitId').val();
	 if(MODE == 'ALL'){
		 var data = {"PN":nPageNo,"fromDate":fromDate,"toDate":toDate,"category":category,"unit":unit,"medcategory":medcategory};		
		}
	else
		{
		 var data = {"PN":nPageNo,"fromDate":fromDate,"toDate":toDate,"category":category,"unit":unit,"medcategory":medcategory};
		} 
	var url = "getAmeReportList";
...
{	
	...
	var dataList = jsonData.data;
	 ...
	 for(item in dataList){
	    	  ...
	    	  htmlTable = htmlTable + "<td >" + dataList[item].height + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].idealWeight + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].weight + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].chest + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].waist + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].pulse + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].bp + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].tabDate + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].ttDate + "</td>";
	    	 /* htmlTable = htmlTable + "<td >" + dataList[item].dv + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].nv + "</td>"; */
	    	  htmlTable = htmlTable + "<td >" + dataList[item].pet + "</td>";
	    	  htmlTable = htmlTable + "<td >" + dataList[item].medicalCategory + "</td>";
	...
	if(jsonData.count == 0 || typeof jsonData.count == 'undefined')
		{
		htmlTable = htmlTable+"<tr ><td colspan='17'><h6>No Record Found</h6></td></tr>";
...
function makeUrl(){
	...
	var toDate = $j('#toDate').val();
	var unit = $j('#unitId').val();
	var empCategory = $j('#category').val();
	var medcategory = $j('#medcategory').val();
	...
	var url="${pageContext.request.contextPath}/report/printAMEReport?fromDate="+fromDate+"&toDate="+toDate+"&unitId="+unit+"&empCategory="+empCategory+"&medcategory="+medcategory;
	openPdfModel(url);
}
</script>
</html>
