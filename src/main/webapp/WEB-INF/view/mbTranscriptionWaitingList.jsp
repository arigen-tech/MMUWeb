...
function getCommandList(MODE)
{
	...
	var searchService = $j('#serviceNo').val();
	 ...
	 if(MODE == 'ALL'){
			var data = {"pageNo":nPageNo,'employeeId' : <%=userId%>, 'hospitalId':""+<%=hospitalId%>,'flagForList':'tem'};			
		}
	else
		{
			var data =  {"pageNo":nPageNo, "serviceNo":searchService,"employeeId":<%=userId%>, "hospitalId":""+<%=hospitalId%>,'flagForList':'tem'}; 
		} 
...
{	 
	...
	for(i=0;i<dataList.length;i++)
		{		
			...
			if(dataList[i].visitStatus=="m")
			{
			htmlTable = htmlTable +"<td style='width: 150px;'>New</td>";
			}
			else if(dataList[i].visitStatus=="c")
			{
...
function executeClickEvent(visitId,data)
{
	...
	for(j=0;j<data.data.length;j++){
		if(visitId == data.data[j].visitId){
			...
			visitStatus = data.data[j].visitStatus;
			departmentId = data.data[j].departmentId;
			examStatus=data.data[j].examStatus;
			visitFlag=data.data[j].visitFlag;
		}
	}
	rowClick(visitId,patientId,patinetname,age,gender,rankName,meTypeName,mbTypeCode,visitStatus,departmentId,visitFlag);
}
...
function rowClick(visitId,patientId,patinetname,age,gender,rankName,meTypeName,status,departmentId){
		...
		var countCheckBox=0;
	    if(mbTypeCode=="MB15" && visitFlag=="R")
	    {	
             window.location.href = "mbTranscriptionDetailsForm15?visitId="+visitId+"&age="+age;
	    }
	    else if(mbTypeCode=="MB15" && (visitFlag=="L" ||visitFlag=="A") && visitStatus=="m")
	    {	
	    ...
	    {
	    	window.location.href = "mbTranscriptionDetailsForm16?visitId="+visitId+"&age="+age;	
	    }
	    else
	    {
	    	window.location.href = "mbTranscriptionDetailsForm15?visitId="+visitId+"&age="+age;
		...
		<div class="content-page">
			...
			<div class="">
				...
				<div class="container-fluid">
					<div class="internal_Htext">Transcription Entry(Medical Board Waiting List)</div>
					 <form:form name="submitMedicalExamByMo" id="submitMedicalExamByMo" method="post"
							 ...
							 <input type="hidden" name="opdPatientDetailId" id="opdPatientDetailId" value="${visitId}"/> 
							...
							<div class="card">
								<div class="card-body">
										...
										<input type="button" id="newEntry"
														...
														onClick="return newMbEntryForm('newEntry');" value="New Entry" />
										</div>
