...
function getCommandList(MODE)
{
	...
	var searchService = $j('#serviceNo').val();
	 ...
	 if(MODE == 'ALL'){
			var data = {"pageNo":nPageNo, "serviceNo":"",'employeeId' : <%=userId%>, 'userId' : <%=userId%>,'hospitalId':""+<%=hospitalId%>,'flagForList':'b'};			
		}
	 else if(MODE == 'FILTER'){
			 
			var data =  {"pageNo":nPageNo, "serviceNo":searchService,"employeeId":<%=userId%>, 'userId' : <%=userId%>,"hospitalId":""+<%=hospitalId%>,'flagForList':'b'};
		}
...
function rowClick(visitId,patientId,patinetname,age,gender,rankName,meTypeName,status,departmentId,meTypeCode){
		...
		if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() === resourceJSON.meForm18){
			 window.location.href = "validateMAWaitingDetailAFMSF18?visitId="+visitId+"&age="+age+"&userId="+<%=userId%>+"&patientId="+patientId;	
		}
		else if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() == resourceJSON.meForm3A){
			window.location.href = "validateMAWaitingDetail3A?visitId="+visitId+"&age="+age+"&userId="+<%=userId%>+"&patientId="+patientId;	
		}
		else{
		 	window.location.href = "validateMAWaitingDetail?visitId="+visitId+"&age="+age+"&userId="+<%=userId%>+"&patientId="+patientId;	
		}
		}
		...
		<div class="content-page">
			...
			<div class="">
				<div class="container-fluid">
					<div class="internal_Htext">Medical Exam Waiting List(MA)</div>
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
