		...
		<div class="content-page">
			...
			<div class="">
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-12">
						<form:form id = "pendingSampleValidationDetailsForm" name="pendingSampleValidationDetailsForm" action="" method="POST">
										...
										<table class="table table-hover table-striped table-bordered m-t-10" id="tblId">
											<thead class="bg-primary">
												<tr>
													...
													<th>Sample</th>
													<th>Date</th>
													<th><div class="form-check form-check-inline cusCheck m-l-10"><input type="checkbox" id="acceptedChkbox" name="acceptedChkbox" class="dependCheck form-check-input" checked="checked" multiple="multiple"  /><span class="cus-checkbtn"></span> <label class="form-check-label">Accepted</label></div></th>
													...
													<th>Sample Collection Remarks</th>
...
$j(document).ready(function()
		{
				...
				if(dataList1[i].reason == undefined){
					htmlTable = htmlTable +"<td style='width: 150px;'><input  class='form-control border-input' type='text' name='reason' id='reason"+counter+"'value='' autocomplete='off' readOnly='readOnly'/></td>";
				}else{
					htmlTable = htmlTable +"<td style='width: 150px;'><input  class='form-control border-input' type='text' name='reason' id='reason"+counter+"'value='"+dataList1[i].reason+"' autocomplete='off' readOnly='readOnly'/></td>";
				}	
...
function multiselectAcceptCheckBox(source){
 ...
 function multiselectRejectedCheckBox(source){
    ...
    function enableAndDisable(){
    	var count = $('#tblListOfSampleValidatedList tr').length;
		  for(var j=1;j<=count;j++){
				if((document.getElementById("acceptedChkbox"+j).checked == true) || (document.getElementById("rejectedChkbox"+j).checked == true)){
					//$('#reason'+j).attr('readonly', false);
					$('#additionalRemarks'+j).attr('readonly', false);
					/* $('#reason'+j).val('');
					$('#additionalRemarks'+j).val(''); */
 ...
 function checkForReject(val){
	...
	function submitSampleValidationDetails(){
		...
		var flag = checkedAcceptedAndRejected();
		if(flag == false){
			return;
		}
		checkedRejected();
		$('#submitbtnId').prop("disabled", true);
		document.pendingSampleValidationDetailsForm.action='${pageContext.request.contextPath}/lab/submitSampleValidationDetails';
		document.pendingSampleValidationDetailsForm.method='POST'
		document.pendingSampleValidationDetailsForm.submit();
	}
		...
		$('#tblListOfSampleValidatedList tr').each(function(i, el) {
			...
			//alert("id :: "+id);
			 if (document.getElementById(id).checked == true) {
				...
				// alert("aInvestigationid :: "+aInvestigationid);
				 ...
				 subchargeCodeId = $(this).find("td:eq(9)").find("input:eq(0)").attr("id")
