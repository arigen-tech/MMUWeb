...
function getDgFundAllcationHdDt(data) {
	...
	var approvedFlag=0;
	var getTotalAvailableBalance=0;
	var getTotalUtilizationBalance=0;
	if (data != null && data.length > 0) {
		for (var i = 0; i < data.length; i++) {
			var ucUploadHdId=data[i].ucUploadHdId;
			var upssId=data[i].upssId;
			var phase=data[i].phase;
			var uploadDate=data[i].uploadDate;
			var fileName=data[i].fileName;
			var createdBy=data[i].createdBy;
			var remarks=data[i].remarks;
			var finanicalId=data[i].finanicalId;
			var ucUploadDtId=data[i].ucUploadDtId;
			var headTypeId=data[i].headTypeId;
			var availableBalance=data[i].availableBalance;
			var certificateNo=data[i].certificateNo;
			var availableUtilization=data[i].availableUtilization;
			...
			$("#financialYear").val(finanicalId);
			$("#fundLetterName").val(data[i].fileName);
			$("#upss").val(upssId);
			$("#phaseIdVal").val(phase);
			$("#dateOfUpload").val(uploadDate);
			$("#remarks").val(remarks);
			$("#ucUploadHdId").val(ucUploadHdId);
			...
			if(status=="S")
			{
				...
				allocationGridValue += '<td><input type="text" id="availableBalance" onchange="calculateAvailableBal(this)" maxlength="12" value="'+availableBalance+'" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" name="availableBalance" class="form-control"/><input type="hidden"  name="ucUploadDtId" value="'+ data[i].ucUploadDtId+ '" id="ucUploadDtId'+data[i].ucUploadDtId+ '"/></td>';
				allocationGridValue += '<td><input type="text" id="availableUtilization" onchange="calculateSum(this)" maxlength="12" value="'+availableUtilization+'" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" name="availableBalance" class="form-control"/><input type="hidden"  name="ucUploadDtId" value="'+ data[i].ucUploadDtId+ '" id="ucUploadDtId'+data[i].ucUploadDtId+ '"/></td>';
				allocationGridValue += '<td><button type="button" type="button" class="btn btn-primary buttonAdd noMinWidth" value="" button-type="add" onclick="addHeadAmountRow()"></button><button type="button" name="delete" value="'+ data[i].ucUploadDtId+ '" id="deleteMC" class="buttonDel btn btn-danger noMinWidth" button-type="delete" onclick="removeHeadAmountRow(this,this.value)"></button></td>';	
			}
			else
			{	
			allocationGridValue += '<tr>';
			...
			//allocationGridValue += '<td><select class="form-control" id="headType" name="headType"><option value="'+headTypeId+'">Select</option></select></td>';
			allocationGridValue += '<td ><select name="headType" class="form-control"  onchange="checkDuplicateRecord(this);" id="headType'+ count + '" ';
			allocationGridValue += 'class="medium" disabled>';
			var dispStock = data[i].dispUnitId;
			allocationGridValue += '<option value=""><strong>Select</strong></option>';
			...
			allocationGridValue += '</select>';
			allocationGridValue += '</td>';
			allocationGridValue += '<td><input type="text" id="availableBalance" maxlength="12" onchange="calculateAvailableBal(this)" value="'+availableBalance+'" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" name="availableBalance" class="form-control" disabled/><input type="hidden"  name="ucUploadDtId" value="'+ data[i].ucUploadDtId+ '" id="ucUploadDtId'+data[i].ucUploadDtId+ '"/></td>';
			allocationGridValue += '<td><input type="text" id="availableUtilization" onchange="calculateSum(this)" maxlength="12" value="'+availableUtilization+'" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" name="availableBalance" class="form-control" disabled/><input type="hidden"  name="ucUploadDtId" value="'+ data[i].ucUploadDtId+ '" id="ucUploadDtId'+data[i].ucUploadDtId+ '"/></td>';
			allocationGridValue += '<td><button type="button" type="button" disabled="disabled"	class="btn btn-primary buttonAdd noMinWidth" value="" button-type="add" onclick="addHeadAmountRow()"></button><button type="button" name="delete" value="'+ data[i].ucUploadDtId+ '" disabled="disabled" id="deleteMC" class="buttonDel btn btn-danger noMinWidth" button-type="delete" onclick="removeHeadAmountRow(this,this.value)"></button></td>';
			}
			count++;
			getTotalAvailableBalance=parseInt(getTotalAvailableBalance)+parseInt(availableBalance);
			getTotalUtilizationBalance=parseInt(getTotalUtilizationBalance)+parseInt(availableUtilization);
		}
		$("#headAmountGrid").html(allocationGridValue);
		$('#availableTotal').val(getTotalAvailableBalance);
		$('#utilizationTotal').val(getTotalUtilizationBalance);
		if(approvedFlag==1)
		{
       	...
       	$("#headAmountGrid").find("input,button,textarea,select").attr("disabled", "disabled");
...
function getDownloadData()
{
	...
	window.open("${pageContext.servletContext.contextPath}/audit/download?name="+namVal+"&type=UC_Upload_Document&keys="+$('#certificateNo').val(), '_blank').focus();	
}
...
function removeHeadAmountRow(val,dtValue){
	...
	if($('#headAmountGrid tr').length > 1) {
		...
		var urldeleteGridRow = window.location.protocol + "//"+ window.location.host + "/" + accessGroup+ "/audit/deleteUCUploadDtDetails";

		var data = {
			"ucUploadDtId" : dtValue
		};
		$.ajax({
			type : "POST",
...
function refreshDelete(val)
{
	var currentAvailableVal=$(val).closest('tr').find("td:eq(1)").find(":input").val(); 
	 var currentRowId= $(val).closest('tr').find("td:eq(0)").find(":input").attr("id");
...
var currentAvailableValU=$(val).closest('tr').find("td:eq(2)").find(":input").val(); 
	 var currentRowIdU= $(val).closest('tr').find("td:eq(2)").find(":input").attr("id");
...
function saveSubmitFundAllocationFunction(val) {
	...
	//////////////////////////////table validation part ////////////////	
	  ...
	  /*   if(!$('#letterUpload').val()){
		...
		var availableBalance = $tds.eq(1).find(":input").val();
		var availableUtilization = $tds.eq(2).find(":input").val();
	    var ucUploadDtId=$tds.eq(2).find("input:eq(1)").val();
	
	dataDt={
			'headTypeId':headTypeId,
			'availableBalance' : availableBalance,
			'availableUtilization':availableUtilization,
			'ucUploadDtId':ucUploadDtId
		} 
...
function checkDuplicateRecord(item)
{
	...
	$('#headAmountGrid tr').each(function(i, el) {
		idforHead= $(this).find("td:eq(0)").find("input:eq(0)").attr("id") 
	if(document.getElementById('headType1').value!= '')
	{
	var $tds = $(this).find('td')
		...
		<div class="content-page">
			...
			<div class="container-fluid">
               ...
               <form name="headMainData" id="headMainData" method="post" enctype='multipart/form-data'
				...
				<div class="row">
				<input type="hidden"  name="ucUploadHdId" value="" id="ucUploadHdId" />
						...
						<div class="card">
							<div class="card-body">
								...
								<div class="row">
									...
									<div class="col-md-4">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<select class="form-control" name="financialYear" id="financialYear" disabled>
								...
								<div class="row m-t-20">
	                               	<div class="col-12">
	                               		<h6 class="font-weight-bold text-theme text-underline">UC Upload Details </h6>
	                               	</div>
	                               	...
	                               	<div class="col-md-4">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<select class="form-control" id="upss" name="upss" onchange="checkDuplicateRecord(this);getCityList(this);" disabled>
										 			<option value="">Select</option>
										 		</select>
											</div>
										</div>
									...
									<div class="col-md-4">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<select class="form-control" id="phaseIdVal" name="phase" disabled>
													<option value="">Select</option>
										...
										<div class="form-group row">
											...
											<div class="col-md-7">
												<div class="dateHolder">
													<input type="text" name="dateOfUpload" id="dateOfUpload" class="form-control"   onchange="checkFutureDate()" placeholder="DD/MM/YYYY" disabled>
												</div>
											</div>
										</div>
									</div>
                                  ...
                                  <div id="resultnavigation">
                                    ...
                                    <table id="tbl" class="table table-striped table-hover table-bordered ">
                                     ...
                                     <tbody id="headAmountGrid">
										 <tr>
										 	...
										 	<td>
										 		<select class="form-control" id="headType" name="headType" onchange="checkDuplicateRecord(this);" disabled>
										 			<option value="">Select</option>
										 		</select>
										 	</td>
										 	<td>
										 		<input type="text" id="availableBalance" maxlength="12" name="availableBalance" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" class="form-control" disabled>
									 		</td>
									 		<td>
										 		<input type="text" id="availableUtilization" maxlength="12" name="availableUtilization" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" class="form-control" disabled>
									 		</td>
									...
									<div class="col-md-3">
										<div class="form-group row">
											...
											<div class="col-md-10">
												<input type="text" id="availableTotal" onchange="calculateAvailableBal(this)" name="availableTotal" class="form-control" readonly/>
											</div>
									...
									<div class="col-md-3">
										<div></div>
											<div class="col-md-10">
												<input type="text" id="utilizationTotal" onchange="calculateUtiliztionBal(this)" name="utilizationTotal" class="form-control" readonly/>
											</div>
									...
									<div class="col-md-4">
										<div class="form-group row">
											...
											<div class="col-md-7">
												...
												<button name="data" class="btn btn-primary" type="button" id="downloadBill" onclick="getDownloadData()">Download UC Certificate</button>
											</div>
										</div>
									</div>
