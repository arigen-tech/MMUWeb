...
%>
	...
	function getPatientPrescription() {
		...
		$('#mobileNo').val(response.headerInfo.mobileNo);
		...
		patientTypeFlag = response.headerInfo.patientTypeFlag;
		patientId = response.headerInfo.patientId;
		$.ajax({
			type : "POST",
		...
		for (var i = 0; i < detailList.length; i++) {
			...
			html += '<td><div class="autocomplete forTableResp"><input type="text" class="form-control width330" onkeypress="commonForAutoPopulate(this)" onblur="populatePVMS(this.value,1,this);" value="'
			...
			html += '<td class="width50"><input type="text" class="form-control width60" value="'+detailList[i].disp_stock+'" readonly></td>';
			html += '<td style="display: none";><input type="hidden" class="form-control width60" value="'+detailList[i].store_stock+'" readonly/></td>';
	...
	function addNomenclatureRow(item) {
		...
		aClone.find("td:eq(5)").find('select').html(optionList);
...
<script type="text/javascript">
	...
	function goBack() {
		window.location = "pendingPrescriptionJSP";
	}
	...
	function issueMedicine(){
		...
		console.log("data is "+JSON.stringify(paramArray));
		var params = {
			"dtData": paramArray,
			"mmuId": <%= mmuId %>,
			"department_id": <%= departmentId %>,
		...
		$.ajax({
			type : "POST",
			contentType : "application/json",
			url : "issueMedicineFromDispensary",
			data :input,
			dataType : 'json',
			timeout : 100000,
			success : function(res) {
		...
		<div class="content-page">
			...
			<div class="">
				<div class="container-fluid">
					<div class="internal_Htext">Patient Prescription</div>
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									...
									<div class="row">
										<div class="col-md-4">
											<div class="form-group row">
												<div class="col-md-5">
													<label for="service" class="col-form-label">Mobile
														No.</label>
												</div>
												<div class="col-md-7">
