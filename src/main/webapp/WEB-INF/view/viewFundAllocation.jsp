...
function GetDistrictList(phaseVal,data){
	...
	jQuery.ajax({
	    ...
	    success: function(result){
	    	...
	    	if(masUpss!=null&&masUpss!=undefined){
	    		setTimeout( function() { getDgFundAllcationHdDt(data); }, 1000);
	    	}
...
function getCityListPOP(item,id){
	var currentRowId= id;//$(item).closest('tr').find("td:eq(1)").find(":input").attr("id");
	$j('#'+currentRowId).empty();
	// $j("#mmuIds").empty();
	//var districtId=$j("#upss option:selected").val();
	//var districtId=$(item).closest('tr').find("td:eq(0)").find(":input").val();
	jQuery.ajax({
	    ...
	    crossDomain:true,
	    url: "${pageContext.servletContext.contextPath}/master/getAllCity",
	    data: JSON.stringify({"PN" : "0","districtId": item}),
	    contentType: "application/json; charset=utf-8",
	    dataType: "json",
	    success: function(result){
	    	var combo = "" ;
...
function removeHeadAmountRow(val,dtValue){
	...
	if($('#headAmountGrid tr').length > 1) {
		...
		var urldeleteGridRow = window.location.protocol + "//"+ window.location.host + "/" + accessGroup+ "/audit/deleteFundAllocationDtDetails";

		var data = {
			"fundAllocationDtId" : dtValue
		};
		$.ajax({
			type : "POST",
...
//////////////////////////////table validation part ////////////////	
   ...
   /*  if(!$('#letterUpload').val()){
	...
	var fundAllocationDtId=$($tds).closest('tr').find("td:eq(3)").find("input:eq(1)").val();
	...
	dataDt={
			...
			'cityId':cityId,
			'amount' : amount,
			'fundAllocationDtId':fundAllocationDtId
		} 
    ...
    var dataJSON = {
            ...
            'userId':$('#userId').val(),
            'fundAllocationHdId': $('#fundAllocationHdId').val(),
...
function getDgFundAllcationHdDt(data) {
					...
					if (data != null && data.length > 0) {
						for (var i = 0; i < data.length; i++) {
							var fundAllocationHdId=data[i].fundAllocationHdId;
							...
							var allocatedAmount=data[i].allocatedAmount;
							...
							$("#fundAllocationHdId").val(fundAllocationHdId);
							...
							$("#phase").val(phaseVal);
							var cityIddddd='cityId'+count;
							//getCityListPOP(districtId,cityIddddd);
							...
							var dispStock = data[i].dispUnitId;
							var selectFreCity = "";
							/* $.each(masCity, function(ijk, item2) {
	
								if (cityId == item2.cityId) {
									selectFreCity = "selected";
									//getCityList(this);
								} else {
									selectFreCity = "";
								}
								allocationGridValue += '<option ' + selectFreCity
										+ ' value="' + item2.cityId + '">'
										+ item2.cityName + '</option>';
							}); */
							/* allocationGridValue += '<option value=""><strong>Select</strong></option>';*/
...
function checkDuplicateRecord(item)
{
	 ...
	 if(recordValue!=""&&recordValue!=null&&recordValue!=undefined){
		...
		var data = {
			"fundAllocationDtId" : recordValue
		};
		$.ajax({
			type : "POST",
			contentType : "application/json",
...
function submitDialouge()
{
	...
	$('#msgForSaved').hide();
	$('.modal-backdrop').hide();
	window.location = 'fundAllocationList';
...
function printReport(){
	var fundBillDetailId=$('#fundAllocationHdId').val();
		...
		<div class="content-page">
			...
			<div class="container-fluid">
               ...
               <form name="headMainData" id="headMainData" method="post" enctype='multipart/form-data'
				...
				<input type="hidden"  name="fundAllocationHdId" value="" id="fundAllocationHdId" />
				<input type="hidden"  name="createdUserId" value="" id="createdUserId" />
				<input type="hidden"  name="fundLetterName" value="" id="fundLetterName" />
				<input type="hidden"  name="createdOn" value="" id="createdOn" />
						...
						<div class="card">
										...
										<div class="form-group row">
											...
											<div class="col-md-7">
												<div class="dateHolder">
													<input type="text" name="dateOfUpload" id="dateOfUpload" onchange="checkFutureDate()" class="calDate form-control" placeholder="DD/MM/YYYY">
												</div>
											</div>
										</div>
									</div>
                                  ...
                                  <div id="resultnavigation">
                                    ...
                                    <table id="tbl" class="table table-striped table-hover table-bordered ">
                                     ...
                                     <tbody id="headAmountGrid">
										 <tr>
										 	<td>
										 		<select class="form-control" id="upss" name="upss" onchange="checkDuplicateRecord(this);">
										 			<option value="">Select</option>
										 		</select>
										 	</td>
										 	<td>
							...
							<div class="row"> 
                               <div class="col-md-12 text-right">														 
                                    ...
                                    <button type="button" id="reportBtn" value="" onclick="printReport()" class="btn btn-primary">Report</button>
