...
function getCommandList(MODE)
{
	...
	var searchService = $j('#serviceNo').val();
	 ...
	 if(MODE == 'ALL'){
			var data = {"pageNo":nPageNo,"serviceNo":"",'employeeId' : <%=userId%>, 'hospitalId':""+<%=hospitalId%>,'flagForList':'a'};			
		}
	    else if(searchService!="")
		{
		   nPageNo = 1;
		    var data =  {"pageNo":nPageNo, "serviceNo":searchService,"employeeId":<%=userId%>, "hospitalId":""+<%=hospitalId%>,'flagForList':'a'};
		} 
	   else
		{
			var data =  {"pageNo":nPageNo, "serviceNo":searchService,"employeeId":<%=userId%>, "hospitalId":""+<%=hospitalId%>,'flagForList':'a'}; 
		} 
	var url = "getMBWaitingList";
...
{	 
	...
	for(i=0;i<dataList.length;i++)
		{		
			...
			htmlTable = htmlTable +"<td style='width: 150px;'>"+dataList[i].visitDate+"</td>";
			//htmlTable = htmlTable +"<td style='width: 100px;'><button type='button' id='closeMb' onClick='event.stopPropagation(); showCloseMessage("+dataList[i].visitId+")' name='closeMb' class='btn btn-primary btn-sm'>Close</button></td>";
 ...
 */	   localStorage.status=status;
       window.location.href = "preliminaryMBWaitingDetailMO?visitId="+visitId+"&age="+age;
	//}
}
		...
		<div class="content-page">
			...
			<div class="">
				...
				<div class="container-fluid">
					<div class="internal_Htext">Preliminary Investigation MB Waiting List(MO)</div>
									...
									<table class="table table-hover table-striped table-bordered m-t-10">
										<thead class="bg-primary">
											<tr>
												...
												<th><button type='button' id='selectAllCheckBox' onClick='selectAllCheckForCloseMb()' name='selectAllCheckBox' class='btn btn-primary btn-sm'>Select All</button></Button></th>
											<!-- 	<th>Validation</th> -->
