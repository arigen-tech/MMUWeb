	...
	<div id="wrapper">
		<div class="content-page">
			...
			<div class="container-fluid">
				...
				<div class="internal_Htext">Pending Audit List of  Attendance</div>
 ...
 {	
 	...
 	for(i=0;i<dataList.length;i++)
 		{	 		
 			...
 			htmlTable = htmlTable +"<td style='width: 150px;'><textarea class='form-control width150' id='remarks"+counter+"' name='remarks"+counter+"'></textarea></td>"; 
...
/*  function executeClickEvent(Id)
 ...
 function showResultPage(pageNo) 	
 {
  ...
  function saveAttendanceAudit(item){
		
		
		var auditAttendanceData = [];  
		var auditAttendance='';
		var idforTreat='';
		
		$('#tblPendingAttnList tr').each(function(i, el) {
			idforTreat= $(this).find("td:eq(0)").find("input:eq(0)").attr("id") 
	    var $tds = $(this).find('td')
	  		
		var attnId = $tds.eq(0).find(":input").val();
		var distance = $tds.eq(1).find(":input").val();
		var time = $tds.eq(2).find(":input").val();
		var status = $tds.eq(9).find(":input").val();
		var remarks = $tds.eq(10).find(":input").val();
		...
		auditAttendanceData.push(auditAttendance);
	});

		
		var params = {
				"auditAttendanceData":auditAttendanceData
  ...
  function backToWaitingList(){
		window.location = "pendingListAttendance";
	}
</script>
