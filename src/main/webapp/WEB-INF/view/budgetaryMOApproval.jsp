		...
		<div class="content-page">
			<div class="">
				<div class="container-fluid">
					<div class="internal_Htext"> BUDGETARY APPROVAL(MO)</div>
											...
											<tbody id="nomenclatureGrid">
												<tr>
													...
													<td><input type="text" id="lpunitrate1"
														 name="lpunitrate"  onkeypress="if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;""
										...
										<div class="col-md-12">
											...
											<div class="btn-right-all">
												...
												<div class="button-list">
													<input type="submit" class="btn btn-primary " name="co" value="FORWARD TO CO" id="fco" onclick="return submitForm('co');" />
													<input type="submit" name="lpc" id="lpc" class="btn btn-primary" value="FORWARD TO LPC" onclick="return submitForm('lpc');" />
														<a  class="buttonDel  btn btn-primary" role="button"
															href="${pageContext.request.contextPath}/lpprocess/budgetaryApprovalList">Close</a>
...
$j(document).ready(
  function getMastStoreItem(){
		if(<%= departmentId %>!=0){
			...
			$("#lpc").attr("disabled", false);
			$("#fco").attr("disabled", false);
			}else{
...
function submitForm(value) {
	 var r=true;
	 var approxCost = $j("#approxCost").val();
	  ...
	  $("#nomenclatureGrid tr").each(function () {
	     ...
	     if (reqQty == 0 ) {
	  	 ...
	  	 if (approxCost > 250000 ) {
		        alert("Approx Cost should not be greater then 250000");
		        flag=1;
		        return false;
		    }
	  	 if(value=='lpc' && approxCost <= 30000){
			 r = confirm("CO approval is required !Still you want to  proceed ?");
			//alert("CO Approval is  Required !Still you want to  proceed ?");
			 
		 }
		 else if(value=='co' && approxCost  > 30000){
			  r = confirm("LPC approval is required !Still you want to  proceed ?");
			//alert("CO Approval is not Required ! Still you want to  proceed ?");
		 }
	    if(flag==1){
	    	return false;
	    }
	    ...
	    setTimeout(function(){ 			 
			 $("#fco").attr("disabled", "disabled");
			 $("#lpc").attr("disabled", "disabled"); 
		 }, 50);
	    }
	    else
	    	return false;
...
function getApproxCost(){
	var approxCost=0;
	  ...
	  $('#approxCost').val( Math.round(approxCost * 100) / 100);
	 return;
}
