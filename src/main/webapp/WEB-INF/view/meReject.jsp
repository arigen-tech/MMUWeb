...
function executeClickEventCommon(visitId,data)
{
	for(j=0;j<data.data.length;j++){
		if(visitId == data.data[j].visitId){
			...
			if(data.data[j].medicalCategory!="" && data.data[j].medicalCategory!='undefined')
...
function rowClickCommon(visitId,patientId,patinetname,rankName,age,gender,mediceExamDate,medicalCategory,meTypeName,approvedBy, status,departmentId,meTypeCode){
		...
		if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() === resourceJSON.meForm18){
			window.location.href = "${pageContext.request.contextPath}/medicalexam/meInboxProcessF18?visitId="+visitId+"&age="+age+"&approvalFlag=y@@reject"+"&userId="+<%=userId%>+"&patientId="+patientId;
		}
		else if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() == resourceJSON.meForm3A){
			window.location.href = "${pageContext.request.contextPath}/medicalexam/meInboxProcess3A?visitId="+visitId+"&age="+age+"&approvalFlag=y@@reject"+"&userId="+<%=userId%>+"&patientId="+patientId;
		}
		else{
			window.location.href = "${pageContext.request.contextPath}/medicalexam/meInboxProcess?visitId="+visitId+"&age="+age+"&approvalFlag=y@@reject"+"&userId="+<%=userId%>+"&patientId="+patientId;	
		}
 }
		...
		<div class="content-page">
			...
			<div class="">
				<div class="container-fluid">
					<div class="internal_Htext">Rejected List</div>
