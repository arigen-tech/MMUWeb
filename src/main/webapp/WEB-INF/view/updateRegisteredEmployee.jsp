...
$j(document).ready(function()
		{
            ...
            var data = ${empData};
             ...
             if (data.data.encodedStringImage != null && data.data.encodedStringImage !='' && data.data.encodedStringImage.length > 100) {
            	 ...
            	 profileImage.setAttribute('src', "/MMUWeb/resources/images/no-photo.jpg");
             }
             ...
             var counter =1;
              ...
              if(qList.length==0){
            	  htmlTable = htmlTable+"<tr id='' >";			
     				htmlTable = htmlTable +"<td class='width60'>"+counter+"<input  class='form-control border-input' type='hidden' id='qualId"+counter+"' value=''/></td>";
     				htmlTable = htmlTable +"<td style='width: 150px;'><input  class='form-control width200' type='text' name='degree"+counter+"' id='degree"+counter+"'  value='' /></td>";
     				htmlTable = htmlTable +"<td style='width: 150px;'><input  class='form-control minWidth220' type='text' name='instName"+counter+"' id='instName"+counter+"'  value='' /></td>";
     				htmlTable = htmlTable +"<td style='width: 150px;'><input  class='form-control width200' type='text' name='completeionYear"+counter+"' id='completeionYear"+counter+"'  value='' onblur='validateQualYear(this.value)'/></td>";
     				...
     				htmlTable = htmlTable +"<td style='display: none;'><textarea id='base64ForEduDoc"+counter+"' name='base64ForEduDoc' value='' ></textarea></td>"; 
     				htmlTable = htmlTable +"<td style='display: none;'><input type='text' value='' id='eMimeType"+counter+"' name='eMimeType"+counter+"'  /></td>"
     				if(fileString != ''){
     					htmlTable = htmlTable +"<td><a href='#' value='' onclick=viewUploadedFile('') class='btn btn-primary'><i class='fa fa-file-alt'></i> View </a></td>"; 
     				} else {
   		    ...
   		    /* 	if(recordState != 'saved'){
   				...
   				htmlTable = htmlTable +"<td style='width: 150px;'><div class='fileUploadDiv minWidth220;'>"
   				+"<input  class='inputUpload' type='file'   name='eduDocument' id='eduDocument"+counter+"'  value='' />"
   				+"<label class='inputUploadlabel'>Choose File</label>"
   				+"<span class='inputUploadFileName'>No File Chosen</span></div></td>";
   				if(fileString != ''){
   		    ...
   		    for(i=0;i<dList.length;i++){
   				...
   				+"<input  class='inputUpload' type='file' name='requiredDoc' id='requiredDoc"+dCounter+"'  value=''   />"
   				+"<label class='inputUploadlabel'>Choose File</label>"
   				+"<span class='inputUploadFileName'>No File Chosen</span></div></td>";	
   				if(fileString != ''){
   				htmlTable = htmlTable +"<td><a href='#' value='' onclick=viewUploadedFile('" + fileString + "') class='btn btn-primary'><i class='fa fa-file-alt'></i> View </a></td>"; 
		...
		$.ajax({
			...
			success : function(data) {
				if(data.status){
					...
					//resetForm();
					window.location = "getEmpListForEdit";
				}else{
					alert(data.msg);
					$j(item).attr("disabled", false);
				}
...
function addQualificationRow() {
	...
	var htmlTable =htmlTable+"<td class=''></td>";
	
	aClone.find(":input").removeAttr("disabled",false);
	aClone.find('td:eq(7)').html(htmlTable);
...
function addRowForDocument() {
	...
	aClone.find(":input").removeAttr("disabled",false);
	var htmlTable =htmlTable+"<td class=''></td>";
	aClone.find('td:eq(5)').html(htmlTable);
							...
							<div class="col-lg-9 col-sm-12">
									...
									<div class="col-md-6">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<input type="text" id="empMobile" class="form-control" readonly="readonly"/>
											</div>
										</div> 
									</div>
					...
					<!-- 	//////////////////////////////////////////////////////////////Education////////////////////////////////////////////////////////////// -->
									...
									<table class="table table-striped table-hover table-bordered " id="qualificationTable">
		                                 ...
		                                 <tbody id="qualificationGrid">
										 	<tr>
												...
												<td></td>	
												<td style="display:none" class="minwidth120 btn-group-sm">
										 			<button type="button" class="btn btn-sm btn-primary noMinWidth" onclick="addQualificationRow()"><i class="fa fa-plus"></i></button>
						...
						<div class="row m-t-20">
							<div class="col-12 text-right">
								<button type="button" class="btn btn-primary" id="submitBtn" onclick="saveEmployeeRegistration(this,'submitted');">Submit</button>
