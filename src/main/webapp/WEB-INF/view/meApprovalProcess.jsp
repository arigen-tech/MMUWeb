...
function rowClickCommon(visitId,patientId,patinetname,rankName,age,gender,mediceExamDate,medicalCategory,meTypeName,approvedBy, status,departmentId,meTypeCode){
		...
		if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() === resourceJSON.meForm18){
			window.location.href = "${pageContext.request.contextPath}/medicalexam/meInboxProcessF18?visitId="+visitId+"&age="+age+"&approvalFlag=n@@inbox"+"&userId="+<%=userId%>+"&patientId="+patientId;
		}
		else if(meTypeCode!=null && meTypeCode!="" && meTypeCode.trim() == resourceJSON.meForm3A){
			window.location.href = "${pageContext.request.contextPath}/medicalexam/meInboxProcess3A?visitId="+visitId+"&age="+age+"&approvalFlag=n@@inbox"+"&userId="+<%=userId%>+"&patientId="+patientId;
		}
		else{
			window.location.href = "${pageContext.request.contextPath}/medicalexam/meInboxProcess?visitId="+visitId+"&age="+age+"&approvalFlag=n@@inbox"+"&userId="+<%=userId%>+"&patientId="+patientId;	
		}
...
function searchValidateList()
{
	...
	var rhqDropDown = $j('#rhqDropDown').val();
	 if((searchService == undefined || searchService == '') && (rhqDropDown == undefined || rhqDropDown == '')){	
			alert("Please entered  Service No. or Unit");
			return;
		}	
		...
		<div class="content-page">
			...
			<div class="">
				<div class="container-fluid">
					<div class="internal_Htext">Medical Exam Waiting List</div>
