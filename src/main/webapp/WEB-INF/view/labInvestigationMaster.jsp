...
var investId;
var rowclick;
var invType=resourceJSON.mainchargeCodeLab;	
<% 
String userId = "1";
	...
	for(i=0;i<dataList.length;i++)
		{		
		 ...
		 if(dataList[i].status == 'Y' || dataList[i].status == 'y')
			...
			htmlTable = htmlTable +"<td style='width: 150px;'>"+dataList[i].sampleName+"</td>";			
			//htmlTable = htmlTable +"<td style='width: 100px;'>"+dataList[i].collectionName+"</td>";
			htmlTable = htmlTable +"<td style='width: 100px;'>"+dataList[i].uomName+"</td>";
...
function executeClickEvent(invId,data)
{
	...
	for(j=0;j<data.data.length;j++){
		if(invId == data.data[j].investigationId){
			...
			sampleId = data.data[j].sampleId;
			sampleName = data.data[j].sampleName;
			collectionId = data.data[j].collectionId;
			collectionName = data.data[j].collectionName;
			uomId = data.data[j].uomId;
			uomName = data.data[j].uomName;
			result = data.data[j].result;
			minval = data.data[j].minval;
			maxval = data.data[j].maxval;
			loinc = data.data[j].loincCode;
			confidential = data.data[j].confidential;
			iStatus = data.data[j].status;
			flag=  data.data[j].flag;
			pandemicFlag = data.data[j].pandemicFlag;
			pandemicCases = data.data[j].pandemicCases;
			
		}
	}
	rowClick(invId,invName,deptId,deptName,modId,modName,sampleId,sampleName,collectionId,collectionName,uomId,uomName,result,iStatus,loinc,confidential,minval,maxval,flag,pandemicFlag,pandemicCases);
...
function addInvestigation(){
	...
	if(jQuery('#selectSample').find('option:selected').val()==""){
		alert("Please Select sample");
		return false;
	}
	
	if(jQuery('#selectCollection').find('option:selected').val()==""){
		alert("Please Select Container");
		return false;
	}
	
	if(jQuery('#selectUOM').find('option:selected').val()==""){
	...
	if(jQuery('#flag').find('option:selected').val()==""){
		alert("Please Select Flag");
		return false;
	}
	
	if($('#minval').val()!="" && $('#maxval').val() !=""){
		if(parseInt($('#maxval').val()) < parseInt($('#minval').val()))
		{
			alert("Maximum value should not be lesser than Minimum value");
			return false;
		}
	}
	$('#btnAddInvestigation').prop("disabled",true);
	var confidential=$('#confidential').is(':checked') ? $('#confidential').val() :'n';
	var pandemicFlag=$('#pandemicFlag').is(':checked') ? $('#pandemicFlag').val() :'N';
	var params = {
			 'userId':<%=userId %>,	
			 ...
			 'sampleId':jQuery('#selectSample').find('option:selected').val(),
			 'collectionId':jQuery('#selectCollection').find('option:selected').val(),
			 'uomId':jQuery('#selectUOM').find('option:selected').val(),
			 'resultType':jQuery('#resulttype').find('option:selected').val(),
			 'maxValue':$('#maxval').val(),
			 'minValue':$('#minval').val(),
			 'loincCode' : jQuery('#loincCode').val(),
			 'confidential':confidential,
			 'investigationType':invType,
			 'flag' : $j("#flag option:selected").val(),
			 'pandemicFlag':pandemicFlag,
			 'pandemicCases' : $('#pandemicCases').val()
...
function GetSampleList(){
	jQuery.ajax({
	    ...
	    success: function(result){
	    	...
	    	for(var i=0;i<result.data.length;i++){
	    		//comboArray[i] = result.data[i].departmentName;
	    		combo += '<option value='+result.data[i].sampleId+'>' +result.data[i].sampleDescription+ '</option>';
	    		//alert("combo :: "+comboArray[i]);
	    		
	    		
	    	}
	    	jQuery('#selectSample').append(combo);
	    }
...
function GetCollectionList(){
	jQuery.ajax({
	    ...
	    success: function(result){
	    	...
	    	for(var i=0;i<result.data.length;i++){
	    		//comboArray[i] = result.data[i].departmentName;
	    		combo += '<option value='+result.data[i].collectionId+'>' +result.data[i].collectionName+ '</option>';
	    		//alert("combo :: "+comboArray[i]);
	    		
	    		
	    	}
	    	jQuery('#selectCollection').append(combo);
	    }
...
function GetUOMList(){
	jQuery.ajax({
	    ...
	    success: function(result){
	    	...
	    	for(var i=0;i<result.data.length;i++){
	    		//comboArray[i] = result.data[i].departmentName;
	    		combo += '<option value='+result.data[i].UOMId+'>' +result.data[i].UOMName+ '</option>';
	    		//alert("combo :: "+comboArray[i]);
	    		
	    		
	    	}
	    	jQuery('#selectUOM').append(combo);
	    }
...
function rowClick(invId,invName,deptId,deptName,modId,modName,sampleId,sampleName,collectionId,collectionName,uomId,uomName,result,iStatus,loinc,confidential,minval,maxval,flag,pandemicFlag,pandemicCases){	
	 rowclick='y';
	jQuery("#investigationName").val(invName);
	jQuery("#selectDepartment").val(deptId);
	jQuery("#selectModality").val(modId);
	jQuery("#selectSample").val(sampleId);
	jQuery("#selectCollection").val(collectionId);
	jQuery("#selectUOM").val(uomId);
	jQuery("#resulttype").val(result);
	jQuery("#minval").val(minval);
	jQuery("#maxval").val(maxval);
	jQuery("#loincCode").val(loinc);
	jQuery("#flag").val(flag);
	jQuery("#pandemicCases").val(pandemicCases);	
	...
	if(pandemicFlag == 'Y' || pandemicFlag == 'y'){
		jQuery("#pandemicFlag").prop('checked', true);
	
	}
	else{
		jQuery("#pandemicFlag").prop('checked', false);
	}
...
if(iStatus == 'Y' || iStatus == 'y'){
	...
	if(result=='s' || result=='y' || result=='' || iStatus == 'N' || iStatus == 'n'){
		$j('#resultAddBtn').hide();
	}
	else {
		$j('#resultAddBtn').show();
...
{	
	...
	$j('#selectSample').val('');
	$j('#selectCollection').val('');
	$j('#selectUOM').val('');
	$j('#resulttype').val('');
	$j('#minval').val('');
	$j('#maxval').val('');
	$j("#confidential").prop('checked', false);
	$j("#pandemicFlag").prop('checked', false);
	$j('#pandemicCases').val('');
...
/* function generateReport()
{
	...
	document.searchMCCForm.submit(); 
		...
		var popup;
		function showCreateSubInvestigation() {
			  var resultType=$j('select#resulttype option:selected').val();				 
			  var investigationId=investId !='' ? investId :'';
		      var investigationName= invName !='' ? invName : '';
		      var deparmentId= deptId !='' ? deptId : '';
		      var departmentName= deptName !='' ? deptName :'';
		      var modalityId= modId !='' ? modId :'';
		      var modalityName= modName !='' ? modName :'';
		      ...
		      var data={
		        ...
		        if(resultType=='m'){
				popup = window.open("subInvestigationMaster?" + decodeURIComponent($j.param(data)), "popUpWindow", "height=500,width=1200,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes");
        		popup.focus(); 
		        }
		...
		function generateReport()
		 {
			var url="${pageContext.request.contextPath}/report/generateLabInvestigationReport";
			openPdfModel(url);
		 	/* document.searchInvForm.action="${pageContext.request.contextPath}/report/generateLabInvestigationReport";
		 	document.searchInvForm.method="POST";
		 	document.searchInvForm.submit(); */ 
    ...
    <div id="wrapper">
        ...
        <div class="content-page">
            ...
            <div class="">
                ...
                <div class="internal_Htext">Investigation Master(Lab)</div>
												...
												<div class="form-group row">
													...
													<div class="col-4">
														...
														<input type="text" name="searchInvName" id="searchInvName"
															...
															placeholder="Investigation Name" onkeypress="return validateText('searchInvName',100,'Investigation Name');">
                                    ...
                                    <table class="table table-striped table-hover table-bordered ">
                                        <thead class="bg-success" style="color:#fff;">
                                            <tr>
                                                ...
                                                <th id="th3" class ="inner_md_htext"> Sample </th>
                                                <th id="th4" class ="inner_md_htext"> UOM </th>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label for="Sample" class="col-sm-5 col-form-label inner_md_htext">Sample<span class="mandate"><sup>&#9733;</sup></span></label>
                                                            
                                                            <div class="col-md-7">
                                                                <select class="form-control" id="selectSample">
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">Container<span class="mandate"><sup>&#9733;</sup></span></label>
                                                            
                                                            <div class="col-md-7">
                                                                <select class="form-control" id="selectCollection" >
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">UOM<span class="mandate"><sup>&#9733;</sup></span></label>
                                                            
                                                            <div class="col-md-7">
                                                                <select class="form-control" id="selectUOM" >
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <div class="col-md-2" id="resultBtn" style="display:;">
		                                                        <div class="form-group row">
		                                                        	<input id="resultAddBtn" type="button" class="btn btn-primary noMinWidth"  onclick="showCreateSubInvestigation(); return false" value="Add" />
		                                                        </div>
		                                                    </div>
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">Minimum Value</label>
                                                            
                                                            <div class="col-md-7">
                                                                <input class="form-control" type="text" id="minval" placeholder="Minimum Value" onkeypress="return isNumberKey(this, event);" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">Maximum Value</label>
                                                            
                                                            <div class="col-md-7">
                                                                
                                                                <input class="form-control" type="text" id="maxval" onkeypress="return isNumberKey(this, event);" placeholder="Maximum Value"/>
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">LOINC Code</label>
                                                            
                                                            <div class="col-md-7">
                                                                
                                                                <input class="form-control" type="text" id="loincCode" placeholder="LOINC Code" maxlength="20"/>
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">Flag<span class="mandate"><sup>&#9733;</sup></span></label>
                                                    ...
                                                    <div class="form-check form-check-inline cusCheck m-t-5">
															<input class="form-check-input" type="checkbox" name="pandemicFlag" id="pandemicFlag" value="Y"/>
															<span class="cus-checkbtn"></span> 
															<label class="form-check-label m-l-5" for="pandemicFlag">Pandemic</label>
														</div>
                                                    ...
                                                    <div class="form-group row">
                                                            ...
                                                            <label class="col-sm-5 col-form-label inner_md_htext">Pandemic Cases</label>
                                                            
                                                            <div class="col-md-7">
                                                                
                                                                <input class="form-control" type="text" id="pandemicCases" onkeypress="return isNumberKey(this, event);" placeholder="Pandemic Cases"/>
