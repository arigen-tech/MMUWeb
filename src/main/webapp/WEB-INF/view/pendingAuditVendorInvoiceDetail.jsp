    ...
    jQuery.ajax({
        ...
        success: function(result){
            if(result && result.vendorInvoiceDetailData && result.vendorInvoiceDetailData.data && result.vendorInvoiceDetailData.data.length > 0){
				...
				var table = document.getElementById("supportingDocs");
				var rowCount = table.rows.length;
				if(rowCount==0){
					$j('#supportingDocs').append('<div class="col-md-10"><label class="col-form-label">Supporting document not uploaded by vendor</label></div>');
				}
...
function submitAuditorForm(val){
	  ...
	  /*  var penaltyMMURemarks=[];
	   ...
	   $('#mmuPenaltyList tr').each(function(i, el) {
			...
			var repeatId = $tds.eq(0).find(":text").attr("id");
				...
				var itemId=$($tds).closest('tr').find("td:eq(6)").find("input:eq(0)").val();
				 var remarksValue=$($tds).closest('tr').find("td:eq(4)").find("textarea:eq(0)").val();
				 var amount=$($tds).closest('tr').find("td:eq(7)").find("input:eq(0)").val();
				 var penaltyAmount=$($tds).closest('tr').find("td:eq(2)").find("input:eq(0)").val();
				 var fileUpload=$($tds).closest('tr').find("td:eq(3)").find("input:eq(0)").val();
				 if(penaltyAmount!="" && fileUpload==""){
					alert("Please upload document against manual penalty")
					return false;
				 }
   ...
   if($('#actionId').val()=="A")
   {
   	lastApprovalMsg='Approved by Auditor on ('+currentdate+')';
  ...
  if(!$('#fileNameAuditor').val()){
       alert('Please select auditor report file!');
       return false;
   }
  if(!$('#auditorRemarks').val()){
      alert('Please enter auditor remarks!');
      return false;
  }
	...
	<div id="wrapper">
		<div class="content-page">
			...
			<div class="container-fluid">
				...
				<div class="internal_Htext">Vendor Invoice Details For Auditor</div>
								...
								<div id="approvingDetailsSection" style="display:none">
                                 ...
                                 <div class="row m-t-10">
									  <div class="col-lg-4 col-sm-6">
                                            <div class="form-group row">
                                                <div class="col-md-5">
                                                    <label class="col-form-label">Auditor's Report</label>
                                                </div>
                                                <div class="col-md-7">
                                                    <div class="fileUploadDiv">
                                                        <input type="file" class="inputUpload" id="fileNameAuditor" name="fileNameAuditor">
									...
									<div class="col-lg-4 col-sm-6">
										<div class="form-group row">
											...
											<div class="col-md-7">
												<div class="dateHolder">
													<textarea name="auditorRemarks"   style="width:600px; height:100px;" maxlength="1000" class="form-control border-input
