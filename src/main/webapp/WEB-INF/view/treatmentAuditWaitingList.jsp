...
String mmuDropdId="";
if (session.getAttribute("mmuDropdId") != null) {
	mmuDropdId = session.getAttribute("mmuDropdId") + "";
}
...
var dropMMUIdCheck=localStorage.mmuDropDownId;
$j(document).ready(function()
		{
	      <%--  var dropDownMMUId=<%=mmuDropdId%>; --%>
	       getMMUList('');
	       
	       if(dropMMUIdCheck==null && dropMMUIdCheck=="")
	       {
	    	   getCommandList('ALL');
	       }
...
function getCommandList(MODE)
{
	...
	var mmuId = $j('#mmuIdList').val();
	 ...
	 if(MODE == 'ALL'){
			...
			var data = {"pageNo":nPageNo,"mobileNo":"",'employeeId' : userId,'opdPre' :"c","exceptionChecked":"",'mmuIdVal':mmuId};			
		}
	    else if(exceptionTypeVal=='withException')
		{
		 var data = {"pageNo":nPageNo,"mobileNo":"","patientName":searchPatient,'employeeId' : userId,'opdPre' :"c",'mmuIdVal':mmuId,"exceptionChecked":"Y","exceptionType":"withException"};
		}
	  else
		{
			var data =  {"pageNo":nPageNo, "mobileNo":mobile_no,"patientName":searchPatient,"employeeId":userId,"exceptionChecked":"","opdPre":"c","mmuIdVal":mmuId,"uhidNo":uhidNo}; 
		} 
...
<%-- function getCityWaitingList()
{
	...
	var data = {"hospitalId":mmuId,"employeeId": <%=userId%>,"cityId":cityIdVal,"opdPre":"c", "patientName":"","pageNo":"1"};
	var bClickable = true;
	var url = "getAuditWaitingList";
...
function getMMUWaitingList()
{
	...
	<%-- <%session.setAttribute("mmuDropdId", mmuIdVal);%> --%>
	 localStorage.mmuDropDownId=mmuIdVal;
	var nPageNo=1;	
	var data = {"hospitalId":mmuId,"employeeId": <%=userId%>,"mmuIdVal":mmuIdVal,"opdPre":"c", "patientName":"","pageNo":"1","exceptionChecked":"","uhidNo":""};
	var bClickable = true;
	var url = "getAuditWaitingList";
	GetOpdJsonData('tblListOfCommand',data,url,bClickable);
}
function getMMUWaitingExceptionList(exceptionValue)
{
	var mmuIdVal=$('#mmuIdList').val(); 
	var nPageNo=1;	
	var data = {"pageNo":nPageNo,"mobileNo":"","patientName":"",'employeeId' : userId,'opdPre' :"c",'mmuIdVal':mmuIdVal,"exceptionChecked":"Y","exceptionType":exceptionValue};
	var bClickable = true;
	var url = "getAuditWaitingList";
	GetOpdJsonData('tblListOfCommand',data,url,bClickable);
}
...
function rowClick(visitId,patientId,tokenNo,mobile_no,patinetname,relation,  rankName,empName,age,gender,mobileNumber){	
	 ...
	 window.location.href = "getOpdPatientMainData?visitId="+visitId+"&visitStatus=com";
...
function searchOpdRecallList()
{
	...
	var mobile_no = $j('#mobile_no').val();
	 var searchPatient = $j('#patientName').val();
	 var uhidNo=$j('#uhidNo').val();
	 if((mobile_no == undefined || mobile_no == '') && (searchPatient == undefined || searchPatient == '') && (uhidNo == undefined || uhidNo == '')){	
...
/* function getCityList(){
	...
	var pathname = window.location.pathname;
		...
		var urlCity = window.location.protocol + "//"
		+ window.location.host + "/" + accessGroup
		+ "/registration/getCityList";
	$.ajax({
		type : "POST",
...
<body>
 ...
 <div class="">
  <div class="container-fluid">
	  ...
	  <div class="internal_Htext">Prescription Audit Waiting List</div>
                            ...
                            <div class="card">
                                <div class="card-body">
									...
									<div class="col-lg-3 col-sm-6">
										...
										<div class="col-lg-3 col-sm-6">
											<div class="form-group row">
												<label class="col-sm-5 col-form-label">UHID No.</label>
												<div class="col-sm-7">
													<input type="text" class="form-control" id="uhidNo" name="uhidNo" placeholder="">
												</div>
											</div>
										</div>
										<div class="col-lg-3 col-sm-6">
									...
									<!-- 	<div class="col-lg-3 col-sm-6">
										<div class="form-group row">
											<div class="col-md-4">
												<div class="form-check form-check-inline cusRadio">
													<input class="form-check-input" id="check0" name="same" type="radio" value="">
													<span class="cus-radiobtn"></span>
													<label class="col-form-label" for="check0">All</label>
												</div>
											</div>
											<div class="col-md-7">
												<div class="form-check form-check-inline cusRadio">
													<input class="form-check-input" id="check1" name="same" type="radio" value="">
													<span class="cus-radiobtn"></span>
													<label class="col-form-label" for="check1">With Exception</label>
												</div>
											</div>
