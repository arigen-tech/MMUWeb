...
@Component
public class PollJobForDateRange implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			LocalDate stDate = LocalDate.of(2021, 04,01 );
			LocalDate eDate = LocalDate.of(2021, 12,01 );
			
			System.out.println("Start Date--> "+stDate);
			
			//LocalDate eDate = stDate.plusMonths(8).plusDays(1);
			System.out.println("End Date--> "+eDate);

			long numOfDays = ChronoUnit.DAYS.between(stDate, eDate);

			List<LocalDate> listOfDates = LongStream.range(0, numOfDays).mapToObj(stDate::plusDays)
					.collect(Collectors.toList());

			System.out.println(listOfDates.size());
			
			for(LocalDate dt:listOfDates) {
				//String yesterday = (dt.minusDays(1)).format(DateTimeFormatter.ISO_DATE);
				...
				ResponseEntity<String> apiResponse = new RestTemplate().exchange(url, HttpMethod.POST, request,
						String.class);

				String json = apiResponse.getBody();
				Map<String, List<HashMap<String, Object>>> responseMap = mapper.readValue(apiResponse.getBody(),
						Map.class);
