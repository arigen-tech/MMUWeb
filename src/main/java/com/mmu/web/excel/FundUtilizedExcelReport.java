...
public class FundUtilizedExcelReport extends AbstractXlsxView {
	private static String[] FUNDUTLIZEDOPERATION_HEADERS = { "SN", "Vendor", "MMU", "Invoice Date", "Invoice No.", "Invoice Amount",
			"Cleared Amount", "Penalty Amount",  "Upload Date", "Last Approved Status" };
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		String fileName = "FundUtlizedOperation";
		/*
		 * if (flagType.equalsIgnoreCase("C")) { fileName = "MMUClearedInvoiceReport"; }
		 * else if (flagType.equalsIgnoreCase("U")) { fileName =
		 * "MMUUnclearedInvoiceReport"; } else { fileName = "MMUTotalInvoiceReport"; }
		 */
        ...
        for (int i = 0; i < FUNDUTLIZEDOPERATION_HEADERS.length ; i++) {
       	 sheet.setColumnWidth(i, 25*256);
           Cell cell = headerRow.createCell(i);
           cell.setCellValue(FUNDUTLIZEDOPERATION_HEADERS[i]);
    			  ...
    			  for(int i=0;i<jsonArray.length();i++)
	    	        {
	    	            ...
	    	            row.createCell(8).setCellValue(jsonObject1.optString("upload_date"));
	    	            row.createCell(9).setCellValue(jsonObject1.optString("last_approval_status"));
